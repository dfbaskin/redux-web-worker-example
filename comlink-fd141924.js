define("./comlink-fd141924.js",['exports'], function (exports) { 'use strict';
  ...
  const allDataSelector = createSelector(stateSelector, ({ columns, data }) => ({
      columns,
      data
  }));
  ...
  const dataViewSelector = createSelector(stateSelector, state => {
      const { columns, data, scrollTop, scrollLeft, height, width } = state;
      const dataHeight = data.length * CELL_HEIGHT;
      const { bounds, dataWidth } = columns.reduce((acc, { width }) => {
          acc.bounds.push({
              width: width + 1,
              left: acc.dataWidth,
      ...
      const { colStart, colCount } = bounds.reduce((acc, { left, right }, idx) => {
  ...
  const WithSelector = {
      allDataSelector: (selectors) => selectors.allDataSelector,
      dataViewSelector: (selectors) => selectors.dataViewSelector
  };
  ...
  function wrap(ep) {
      return createProxy$2(ep);
  }
  function createProxy$2(ep, path = []) {
      const proxy = new Proxy(function () { }, {
          get(_target, prop) {
              ...
              return createProxy$2(ep, [...path, prop]);
          },
          ...
          apply(_target, _thisArg, rawArgumentList) {
              ...
              if (last === "bind") {
                  return createProxy$2(ep, path.slice(0, -1));
              }
  ...
  exports.allDataSelector = allDataSelector;
