...
define("./app-two.dd229d4d.js",['require', './chunk-fd141924'], function (require, __chunk_1) { 'use strict';
	...
	*/
	...
	function Layout({ renderMenu, renderHeader, renderTable }) {
	...
	function useWindowReisze(resizeCallback, deps) {
	    react_4(() => {
	...
	function GridWorksheetData({ viewData, dataWidth, dataHeight }) {
	    ...
	    useWindowReisze(() => {
	        if (divRef.current) {
	            ...
	            dispatch(__chunk_1.setGridSizeAction({ width, height }));
	        }
	    }, [divRef]);
	    const onScroll = () => {
	        if (divRef.current) {
	            const { scrollLeft, scrollTop } = divRef.current;
	            dispatch(__chunk_1.setGridScrollAction({ scrollLeft, scrollTop }));
	        }
	    };
	...
	function GridRowHeader({ headers }) {
	    return (react.createElement("div", null, headers.map(({ row, top, left, width, height }) => {
	        return (react.createElement("div", { className: "cell", key: row, style: {
	                top,
	                left,
	                width,
	                height
	...
	function useStateSelector(chooseSelector) {
	    const [currentState, setCurrentState] = react_3();
	    const { selectors, subscribe } = react_2(AppStoreContext);
	    react_4(() => {
	        const { unsubscribe } = subscribe(chooseSelector(selectors), setCurrentState);
	        return unsubscribe;
	...
	function DataGrid() {
	    const { viewData, columnHeaders, rowHeaders, dataWidth, dataHeight } = useStateSelector(__chunk_1.WithSelector.dataViewSelector) ||
	        {
	            viewData: [],
	            columnHeaders: [],
	            rowHeaders: [],
	    ...
	    return (react.createElement("div", { className: "grid" },
	        react.createElement(GridWorksheetData, { viewData: viewData, dataWidth: dataWidth, dataHeight: dataHeight }),
	        react.createElement(GridColumnHeader, { headers: columnHeaders }),
	        react.createElement(GridRowHeader, { headers: rowHeaders })));
	}

	var css$2 = ".app-header {\n  display: flex;\n}\n\n.app-header > div:nth-of-type(1) {\n  flex: 0;\n  white-space: nowrap;\n  font-weight: bold;\n}\n\n.app-header > div:nth-of-type(2) {\n  flex: 1;\n  text-align: right;\n}\n\n.app-header > div:nth-of-type(2) button {\n  margin-left: 0.5rem;\n}";
	styleInject(css$2);
	...
	function Header({ title }) {
	    ...
	    react_4(() => {
	        setTimeout(() => {
	            dispatch(__chunk_1.resetDataAction({ rows: 50, cols: 30 }));
	        });
	    }, []);
	    return (react.createElement("div", { className: "app-header" },
	        react.createElement("div", null, title),
	        react.createElement("div", null, sizes.map(({ name, rows, cols }) => (react.createElement("button", { type: "button", onClick: () => dispatch(__chunk_1.resetDataAction({ rows, cols })), key: name }, name))))));
	}
	...
	function DataGridMenu() {
	    ...
	    return (react.createElement("div", { className: "grid-menu" },
	        ...
	        react.createElement("span", null,
	            react.createElement("button", { type: "button", onClick: () => dispatch(__chunk_1.addRandomDataColumnAction()) }, "Random Data"))));
	}
	...
	function initializeWorker() {
	    const worker = new Worker(new URL(require.toUrl('worker-2e2c4ae3.js'), document.baseURI).href);
	    const workerProxy = __chunk_1.wrap(worker);
	    let subscriptionIdCounter = 0;
	    const subscriptions = new Map();
	    const proxyCallback = __chunk_1.proxy(onUpdatedCallback);
	    workerProxy.onUpdate(proxyCallback);
	    return {
	        selectors: {
	            allDataSelector: () => __chunk_1.allDataSelector(__chunk_1.initialState),
	            dataViewSelector: () => __chunk_1.dataViewSelector(__chunk_1.initialState)
	        },
	...
	reactDom.render(react.createElement(AppStoreContext.Provider, { value: appStoreMethods },
	    react.createElement(Main, { title: "Web Worker" })), document.getElementById("root"));
